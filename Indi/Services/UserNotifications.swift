...
class UserNotificationCenter {
    ...
    private func bodyGenerator() -> String {
        ...
        (0...4).forEach { num in
            completionInfo.append(UserData.shared.getExamCompletion(for: num))
        }
        ...
        switch currentStage {
        case 0:
            kitsNames = KitsLibrary.shared.getKitNamesForStudyStage(with: [0])
            examName = StudyStage.getExamName(studyStage: 0)
        case 1:
            kitsNames = KitsLibrary.shared.getKitNamesForStudyStage(with: [1])
            examName = StudyStage.getExamName(studyStage: 1)
        case 2:
            kitsNames = KitsLibrary.shared.getKitNamesForStudyStage(with: [2])
            examName = StudyStage.getExamName(studyStage: 2)
        case 3:
            kitsNames = KitsLibrary.shared.getKitNamesForStudyStage(with: [3,4])
            examName = StudyStage.getExamName(studyStage: 3)
        case 4:
            kitsNames = KitsLibrary.shared.getKitNamesForStudyStage(with: UserData.shared.getSelectedStages())
        ...
        var output = ""
        let name = UserData.shared.getUserName()
