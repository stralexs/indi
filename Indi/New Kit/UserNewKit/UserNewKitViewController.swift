...
final class UserNewKitViewController: UIViewController {
    ...
    private var viewModel: UserNewKitViewModel!
    ...
    private func setupBinders() {
        viewModel.newUserKitName.bind { [weak self] newUserKitName in
            self?.newKitLabel.text = newUserKitName
        }
        viewModel.newUserKitStudyStageName.bind { [weak self] name in
            self?.newKitStudyStageButton.setTitle(name, for: .normal)
        }
    }
    
    //MARK: - Alert Controllers
    @IBAction func newKitButtonIsPressed(_ sender: UIButton?) {
    ...
    @IBAction func studyStageButtonIsPressed(_ sender: UIButton) {
        ...
        let completionHandler = { (action: UIAlertAction) -> Void in
            if let index = newKitStudyStageAlert.actions.firstIndex(where: { $0 == action} ) {
                self.viewModel.newUserKitStudyStage = index - 1
            }
        }
    ...
    @IBAction func addQuestion(_ sender: UIButton) {
        ...
        let addAction = UIAlertAction(title: "Добавить", style: .default) { [weak self] _ in
            let newQuestionResult = self?.viewModel.createNewQuestion(alertController.textFields?[0].text ?? "", alertController.textFields?[1].text ?? "", alertController.textFields?[2].text ?? "")
    ...
    @IBAction func addNewKit(_ sender: UIButton) {
        ...
        if newKitResult == "No kit name" {
        ...
        } else if newKitResult == "No questions" {
            ...
            let okAction = UIAlertAction(title: "Ок", style: .default) { [weak self] _ in
                self?.newKitButtonIsPressed(nil)
            }
